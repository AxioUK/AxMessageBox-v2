VERSION 5.00
Begin VB.UserControl axProgBar 
   ClientHeight    =   780
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   1965
   ScaleHeight     =   780
   ScaleWidth      =   1965
   ToolboxBitmap   =   "axProgBar.ctx":0000
End
Attribute VB_Name = "axProgBar"
...
Public Enum cScrolling
    ccStyleFlat = 0
    ccStyleBorderFlat = 1
    ccStyleUniColorFlat = 2
    ccStyleCustomBrush = 3
    ccScrollingSearch = 4
End Enum
'=====================================================
...
Public Sub DrawProgressBar()
            ...
            DrawFillRectangle TR, IIf(m_Scrolling = ccStyleUniColorFlat, &H0, vbWhite), m_hDC '//--- Draw BackGround
            
            '//-- Draw ProgressBar Style
            If m_Scrolling = ccStyleFlat Then
                   
                 DrawOfficeXPProgressbar

            ElseIf m_Scrolling = ccStyleBorderFlat Then
                 
                 DrawPastelProgressbar
 
            ElseIf m_Scrolling = ccStyleUniColorFlat Then
            
                 DrawMediaProgressbar

            ElseIf m_Scrolling = ccStyleCustomBrush Then
...
Private Sub CalcBarSize()
      ...
      lSegmentWidth = IIf(m_Scrolling = 0, 2, 0) '/-- Windows Default
...
If (m_Scrolling = ccStyleUniColorFlat) Then isAlpha = True
 ...
 If (m_ShowText) Then
      ...
      SetTextColor m_hDC, GetLngColor(IIf(m_Scrolling = ccStyleUniColorFlat, &HC0C0C0, vbBlack))
      
      CalculateAlphaTextRect ThisText             '//--Calculate The Text Rectangle
           
      '//-- If ProgressBar is already over the Text don't draw the old text, just draw the Alpha Text
 ...
 SetBkMode m_hDC, 1                          '//--Transparent Text
        ...
        '//-- This is When the Text is Drawn
            ...
            If (TR.Right * (m_Value / 100)) >= AT.Left Then
                SetTextColor m_hDC, GetLngColor(IIf(m_Scrolling = ccStyleUniColorFlat, ShiftColorXP(m_Color, 80), vbWhite))
                AT.Left = (TR.Right / 2) - ((AT.Right - AT.Left) / 2)
...
HorizontalSearch:
    ...
    For ITemp = 0 To 2
        ...
        With TempRect
          ...
          .Top = 8
          .Bottom = TR.Bottom - 8
          DrawGradient ShiftColorXP(m_Color, 220 - (40 * ITemp)), ShiftColorXP(m_Color, 200 - (40 * ITemp)), .Left, 2, 9, TR.Bottom - 2, m_hDC, True
        End With
...
VerticalSearch:
    ...
    For ITemp = 0 To 2
        ...
        With TempRect
          ...
          .Bottom = .Top + 10
          DrawGradient ShiftColorXP(m_Color, 220 - (40 * ITemp)), ShiftColorXP(m_Color, 200 - (40 * ITemp)), TR.Right, .Top, 0, 9, m_hDC
        End With
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
 ...
 Call PropBag.WriteProperty("Scrolling", m_Scrolling, ccStyleFlat)
 Call PropBag.WriteProperty("ShowText", m_ShowText, False)
 Call PropBag.WriteProperty("Text", m_Text, "Searching...")
...
Scrolling = PropBag.ReadProperty("Scrolling", ccStyleFlat)
ShowText = PropBag.ReadProperty("ShowText", False)
m_Text = PropBag.ReadProperty("Text", "Searching...")
